...
const EditUser = () => {
  ...
  const { id } = useParams();
  const url = `http://localhost:8080/car/update/${id}`;
  ...
  const loadCar = async () => {
    try {
      const response = await axios.get(`http://localhost:8080/car/car/${id}`);
      const carData = response.data;

      console.log("Car Data:", carData);

      setCar_type(carData.car_type);
      setCar_no(carData.car_no);
      setFirst_name(carData.first_name);
      setLast_name(carData.last_name);
      setserviceType(carData.serviceType);
      setPhone_number(carData.phone_number);
